...
public class ProcessorPython {
    ...
    public static ExecutionState compileAndRun( 
        ...
        String cmdExec = String.format( "python %s.py", fileName );
        
        if ( textPane == null ) {
            System.out.printf( "Processing file %s/%s.py\n", baseDir, fileName );
        } else {
            Utils.addFormattedText( 
                    textPane, 
                    String.format( "|-- Processing file %s\\%s.py\n", baseDir, fileName ), 
                    Color.BLACK );
        }

        if ( new File( String.format( "%s/%s.py", baseDir, fileName ) ).exists() ) {
